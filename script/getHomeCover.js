      ...
      .then(function (data) {
        for (var i = 0; i < data.length; i++) {
          if (window.location.href.includes("admin")) {
            if (data[0].image) {
              document.querySelector(".selected_home_cover_img_view").style.display =
                "block";
            ...
            document.getElementById("cover-description").value = data[0].description;
            ...
            document.querySelector(".banner_text_section h2").innerHTML =
              data[0].headerText;
            document.querySelector(".banner_text_section p").innerHTML = data[0].description;
          }
        }
      })
      .catch((err) => {
